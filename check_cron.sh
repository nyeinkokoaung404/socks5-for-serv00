...
echo "Check and add crontab tasks"

if [ "$(command -v pm2)" == "/home/${USER}/.npm-global/bin/pm2" ]; then
  echo "PM2 has been installed and the correct path has been returned. The PM2 keepalive task is enabled."
  ...
  if [ -e "${WORKDIR}/start.sh" ] && [ -e "${FILE_PATH}/config.json" ]; then
    echo "Add crontab restart task for nezha & socks5"
    ...
    echo "Add a crontab restart task for nezha"
    ...
    echo "Add socks5 crontab restart task"
